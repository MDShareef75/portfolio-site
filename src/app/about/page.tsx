    ...
    <div className="min-h-screen relative overflow-hidden bg-gradient-to-br from-[var(--background)] via-[var(--surface)] to-[var(--background)]">
      ...
      <div className="container mx-auto px-4 py-16 relative z-10">
        ...
        <div className="text-center mb-16 mt-16 relative animate-fade-in">
          <h1 className={`text-5xl md:text-6xl font-bold mb-4 text-center bg-gradient-to-r from-[#64ffda] via-blue-400 to-purple-500 text-transparent bg-clip-text animate-gradient drop-shadow-[0_2px_24px_rgba(100,255,218,0.25)]`}>About Us</h1>
          <p className={`text-base md:text-lg max-w-2xl mx-auto mt-2 text-center ${theme === 'light' ? 'text-[var(--text)]' : 'text-[var(--text-secondary)]'}`}>
            Learn more about our mission, values, and the passionate team driving Atoms Innovation forward.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center mb-20 animate-fade-in delay-100">
          <div className="relative group">
            ...
            <div className={`relative backdrop-blur-lg border border-accent/30 shadow-2xl p-8 md:p-10 rounded-2xl z-10 overflow-hidden ${theme === 'light' ? 'bg-[var(--surface)]/95' : 'bg-[var(--surface)]/80'}`}>
              <h2 className={`text-3xl md:text-4xl font-bold mb-6 ${theme === 'light' ? 'text-[var(--accent)]' : 'text-accent'}`}>Who we are</h2>
              <div className={`space-y-4 text-base leading-relaxed text-left md:text-justify ${theme === 'light' ? 'text-[var(--text)]' : 'text-gray-200'}`}>
                ...
                <p className={`font-semibold ${theme === 'light' ? 'text-[#0891b2]' : 'text-accent'}`}>
        ...
        <div className="text-center mb-12 animate-fade-in delay-200">
          <h2 className={`text-4xl md:text-5xl font-bold mb-6 text-center bg-gradient-to-r from-[#64ffda] via-blue-400 to-purple-500 text-transparent bg-clip-text animate-gradient`}>Our Toolkit</h2>
              ...
              <button
                ...
                className={`px-4 py-2 rounded-full transition-all duration-300 ${
                  activeCategory === category
                    ? 'bg-[var(--accent)] text-[var(--background)] font-semibold shadow-lg shadow-[var(--accent)]/20'
                    : 'bg-[var(--surface)]/50 text-[var(--text-secondary)] hover:bg-[var(--surface)] hover:text-[var(--accent)]'
                }`}
              >
                {category}
              </button>
        ...
        <div className="max-w-4xl mx-auto mb-20 animate-fade-in delay-300">
          <h2 className={`text-3xl font-bold text-center mb-12 bg-gradient-to-r from-[#64ffda] via-blue-400 to-purple-500 text-transparent bg-clip-text animate-gradient`}>Founder's Journey</h2>
            ...
            {timeline.map((item: any, index: number) => (
              ...
              >
                ...
                <div className={`w-full md:w-1/2 ${index % 2 === 0 ? 'md:pl-8' : 'md:pr-8'} mb-6 md:mb-0`}>
                  <div className="bg-[var(--surface)]/80 backdrop-blur-md p-6 rounded-xl shadow-lg border border-[var(--surface)]">
                    <div className="flex items-center mb-2">
                      <span className={`w-3 h-3 rounded-full mr-3 ${
                        item.type === 'work' ? 'bg-[var(--accent)]' : 'bg-purple-500'
                      } md:hidden`}></span>
                      <span className="text-gray-400 text-sm">{item.year}</span>
                    </div>
                    <h3 className="text-xl font-semibold text-[var(--text)] mb-1">{item.title}</h3>
                    <p className="text-[var(--accent)] mb-3">{item.company}</p>
                    <p className="text-[var(--text-secondary)] text-sm text-justify">{item.description}</p>
                  </div>
                </div>
                {/* Dot for desktop */}
        ...
        <div className="max-w-4xl mx-auto animate-fade-in delay-400">
          <h2 className={`text-3xl font-bold text-center mb-12 bg-gradient-to-r from-[#64ffda] via-blue-400 to-purple-500 text-transparent bg-clip-text animate-gradient`}>What Drives Us</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 gap-y-8">
            {interests.map((interest: any, index: number) => (
              <div
                key={index}
                className="bg-[var(--surface)]/80 backdrop-blur-md p-6 rounded-xl text-center shadow-lg border border-[var(--surface)] hover:scale-105 transition-transform duration-300"
              >
                <div className="text-4xl mb-3">{interest.icon}</div>
                <p className="text-[var(--text-secondary)]">{interest.text}</p>
              </div>
            ))}
          </div>
        </div>
